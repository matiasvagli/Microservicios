...
# Test: Verifica que el registro funciona aunque falle la publicación del evento
@pytest.mark.asyncio
async def test_register_event_fail(monkeypatch):
    ...
    # Mock: Simula que la publicación de eventos falla
    ...
    # Ejecuta el endpoint como función y verifica que no lanza excepción
    result = await register(user, db=MockDB())
    assert result.email == "testuser@example.com"
    assert result.id == "fakeid123"
    assert result.is_active is True
    # Si quieres que falle, usa pytest.raises(Exception) y verifica el error
